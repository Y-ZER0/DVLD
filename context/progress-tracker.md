...
| Phase 3 — Advanced License Services | In Progress (7.1 + 7.2 + 8.1 + 8.2 + 9.1 + 9.2 done) |
| Phase 4 — Utilities & Reports | Completed (10.1 + 10.2 + 11.1 + 11.2 + 12.1 + 12.2 done) |
...
### Phase 3
- [x] 7.1 — License Renewal & Replacement `[LOGIC]`
- [x] 7.2 — License Renewal & Replacement `[UI]`
- [x] 8.1 — International License Service `[LOGIC]`
- [x] 8.2 — International License Service `[UI]`
- [x] 9.1 — Detain & Release System `[LOGIC]`
- [x] 9.2 — Detain & Release System `[UI]`

### Phase 4
- [x] 10.1 — Driver & License History `[LOGIC]`
- [x] 10.2 — Driver & License History `[UI]`
- [x] 11.1 — Configuration `[LOGIC]`
- [x] 11.2 — Configuration `[UI]`
- [x] 12.1 — Operational Dashboard `[LOGIC]`
- [x] 12.2 — Operational Dashboard `[UI]`
...
### Session 29 — 2026-08-21
**Completed:** **12.1 — Operational Dashboard `[LOGIC]` + 12.2 — Operational Dashboard `[UI]`** — `Dashboard.webp` 1:1: shared `dashboard.dto.ts` (`DashboardSummaryDto` 6 counts + `recentApplications[3]` + `upcomingTestAppointments[5]`); `modules/dashboard/` (`DashboardService` single `GET /dashboard/summary` aggregate — `Applications New`, `TestAppointments today` (`todayStart/End`), `isLocked=false`, `Drivers`, `Licenses isActive`, `Detained isReleased=false` + recent 3 / upcoming 5 joins, `ApplicationType` map), controller/module + `app.module` wiring; web `dashboardKeys`/`dashboard.service`/`useDashboardSummary` (`staleTime 30s`), `StatCard` (`Copy`/`CalendarClock`/`Car`/`Gavel`, `rounded-xl border bg-card p-5 shadow-sm`, View → links), `RecentApplicationsCard` (header + `APPLICATION_TYPE_LABELS` + `New/Completed/Cancelled` pills + mono fees), `UpcomingTestAppointmentsCard` (`Scheduled` pill + `YYYY-MM-DD` + dashed empty), `DashboardPage` (`sm:grid-cols-2 lg:grid-cols-4` + `lg:grid-cols-3` 2:1, skeletons + `role="alert"` retry); replaced `(protected)/dashboard` placeholder; `pnpm --filter @repo/shared build`, `pnpm typecheck 4/4`, `pnpm build 3/3` green (`○ /dashboard 6.09 kB`).
**Decisions made:**
- **Single aggregate** — one `GET /dashboard/summary` not five calls (first-paint fast; contrast with 10.2 four queries).
- **Counts** — `activeApplications` = `ApplicationStatus.New`, `testsToday` = `appointmentDate` today window, `pendingAppointments` = `isLocked=false` (Tests Today subtitle), `activeDrivers`/`activeLicenses` split (Active Drivers subtitle), `detainedLicenses` = `isReleased=false`.
- **Dashboard has no entity** — `DataSource.getRepository` reads only; `ApplicationType` fetched once via `Map`.
- **staleTime 30s** — live feel per build-plan.
- **AGENTS.md §3.1 LOGIC+UI same pass** — explicit user override ("build both tasks").
**Deviations from plan:** LOGIC+UI built together (user directive violates §3.1); otherwise plan-literal single endpoint.
**Known issues / follow-ups:**
- **No API boot + no smoke (session pattern)** — counts/today-window/ordering, `APPLICATION_TYPE_LABELS` completeness, empty states, skeletons unverified at runtime.
- **REVIEWs still owed: 12.2+12.1 now fronts** (single aggregate, today window, `ApplicationType` map, `staleTime`, tokens/icons, hex/comment policy), then 11.2+11.1, 10.2+10.1, 9.2+9.1, 8.1/8.2, 7.1/7.2, 6.1, 5.1, 5.2, backlog.
- Carried: 6.1-vs-7.1 + double-detain concurrency holes; pg deprecation warning; Completed-after-refresh banner gap; Roles column data source; citizen-options 1000-window; TestType descriptions provisional; ui-rules.md neutral-tint pill sync; new tabs pill vs line-variant; queued 7.1 (b) expiry-preservation.
**Start next session with:** Per AGENTS.md §3.1 decide: **REVIEW 12.2+12.1**, or **queued 7.1 (b) preserve old `ExpirationDate`**, or **system-wide REVIEW sweep** (all vertical slices now done — only REVIEWs remain).

### Session 28 — 2026-08-21
**Completed:** **11.2 — Configuration `[UI]`** — `/settings/configuration` per `display system tables for lookups.jpg`: route `app/(protected)/settings/configuration/page.tsx` → `features/configuration/configuration-page.tsx` (H1 + subtitle, `grid lg:grid-cols-2` + full-width); service `configuration.service.ts` (`PATCH /lookup/.../:id` ×3); DTOs `UpdateLicenseClass/ApplicationType/TestTypeRequestDto`; hooks `useUpdateLicenseClass`/`useUpdateApplicationType`/`useUpdateTestType` (invalidate `lookupKeys`); components `ConfigNumberField` (autosave blur/`Enter`, `Escape` revert, `Check`/`LoaderCircle`/`CircleAlert` + `aria-live`, `stripTrailingZeros`, validation `1–120`/`1–50`/`0–99999999.99`), `ApplicationTypesCard` (6 rows + `APPLICATION_TYPE_LABELS` map), `TestTypesCard` (3 rows), `LicenseClassesCard` (7 rows ×3 fields); `pnpm typecheck 4/4 + pnpm build 3/3` green (`○ /settings/configuration 6.75 kB`).
**Decisions made:**
- **`configuration` owns UI, `lookup` owns read keys** — mutations invalidate `lookupKeys` (no new `configurationKeys`; `staleTime 5m` reused; build-plan literal deferred).
- **Plain `<table>` over `DataTable`** — 3/6/7 rows not paginated; lightweight `overflow-x-auto` matches screenshot while keeping `rounded-xl border bg-card shadow-sm` tokens.
- **Per-field autosave via local field state** — no RHF, no debounce, immediate blur/`Enter` per "apply immediately"; client-side `Min`/`Max`/`IsInt`/`maxDecimals` mirrors DTO before PATCH.
- **Label mapping + fee stripping** — enum `NewDrivingLicense` → "New Local Driving License" client-side; `Number("15.00").toString()` → `"15"` for display.
**Deviations from plan:** `configurationKeys` literal not introduced — `lookupKeys` is the existing source of truth for lookup tables; `DataTable` not reused for the same reason (small non-paginated register).
**Known issues / follow-ups:**
- **No API boot + no smoke (session pattern)** — 3 PATCH paths, label mapping, `stripTrailingZeros`, per-field validation, `aria-live` indicators unverified at runtime; 7.1 expiry guards still unverified.
- **REVIEWs still owed: 11.2 `[UI]` + 11.1 `[LOGIC]` now front the queue** (cross-refs: 3 PATCH routes, whitelist, `toFixed(2)`, #28 isolation, autosave, validation ranges, hex/comment policy), then 10.2+10.1, 9.2+9.1, 8.1/8.2, 7.1/7.2, 6.1, 5.1, 5.2, backlog.
- Carried: 6.1-vs-7.1 + double-detain concurrency holes; pg deprecation warning; Completed-after-refresh banner gap; Roles column data source; citizen-options 1000-window; TestType descriptions provisional; ui-rules.md neutral-tint pill sync; new tabs pill vs line-variant; queued 7.1 (b) expiry-preservation.
**Start next session with:** Per AGENTS.md §3.1 decide: **REVIEW 11.2+11.1**, or go to **12.1 — Operational Dashboard `[LOGIC]`** (ARCHITECT `GET /dashboard/summary` aggregate), or finish **7.1 (b) preserve old `ExpirationDate`**.

### Session 27 — 2026-08-21
**Completed:** **7.1 expiry-guard patch (2/3)** — `apps/api/src/modules/licenses/licenses.service.ts`: added `addMonths` import, `renewOrReplace` validates before `findApplicationTypeByTitle` — (1) replacement (`damaged`/`lost`) on expired `expirationDate < today` → `409 'Expired license cannot be replaced — renew it instead'`; (2) renewal on unexpired only within 6 months: `expirationDate >= today && expirationDate > addMonths(today,6)` → `409 'License can only be renewed within 6 months of expiry'` (expired renewal bypasses window); hoisted `now/todayStr` shared with `issueDate`; `pnpm typecheck 4/4 + pnpm build 3/3` green.
**Decisions made:**
- **Window only on unexpired renewals** — expired renews freely; string `YYYY-MM-DD` compare + `addMonths(today,6)` inclusive `>` mirrors repo `>= :today` pattern.
- **Replacement expired = hard 409** — single `reason !== 'renew'` branch, verbatim "renew it instead" hint.
- **Fail-fast before lookup/transaction** — guards run before `findApplicationTypeByTitle` and before `hasOpenDetention` (#32) tx; single `now` avoids midnight drift.
- **Remaining 7.1 (b) deferred** — replacement still mints `addYears(now, validity)` full term; preserving OLD `ExpirationDate` left queued (user's 2-case scope).
**Deviations from plan:** partial queued 7.1 patch — implemented `a)`+`c)` of original `a/b/c` queue, `b)` (preserve old expiry on replacement) left for next patch.
**Known issues / follow-ups:**
- **No API boot + no smoke (session pattern)** — two new 409 paths unverified at runtime; `addMonths` month-length edge correct per date-fns.
- **REVIEWs still owed: 11.1 `[LOGIC]` still fronts** (unchanged), then 10.2+10.1, 9.2+9.1, 8.1/8.2, 7.1/7.2 (now partially patched — 7.1 REVIEW must check new expiry guards), 6.1, 5.1, 5.2, backlog.
- Carried: 6.1-vs-7.1 + double-detain concurrency holes; pg deprecation warning; Completed-after-refresh banner gap; Roles column data source; citizen-options 1000-window; TestType descriptions provisional; ui-rules.md neutral-tint pill sync; new tabs pill vs `TabbedDetailView` line-variant; queued 7.1 (b) expiry-preservation.
**Start next session with:** Per AGENTS.md §3.1 decide: **REVIEW 11.1 `[LOGIC]`**, or go to **11.2 `[UI]`** (ARCHITECT `/settings/configuration` 3 cards + autosave), or finish **7.1 (b) preserve old `ExpirationDate` on replacement**.

### Session 26 — 2026-08-21
**Completed:** **11.1 — Configuration `[LOGIC]`** (full vertical slice: 3 DTOs `UpdateLicenseClassRequestDto` (`minimumAllowedAge`/`defaultValidityLength`/`classFees` all optional via `ValidateIf`; `className` immutable) / `UpdateApplicationTypeRequestDto` (`applicationFees` only) / `UpdateTestTypeRequestDto` (`testTypeFees` only) with `IsInt`/`IsNumber(maxDecimalPlaces:2)` + `Min/Max` and `whitelist+forbidNonWhitelisted`; `ApplicationTypesRepository.findById` added (was missing); `LookupService` 3 methods `updateLicenseClass`/`updateApplicationType`/`updateTestType` — `findById` 404, empty-body 400, `toFixed(2)` for decimal columns, `save`+`toDto` (writes affect only lookup row, #28); `LookupController` 3 routes `PATCH lookup/license-classes/:id`, `PATCH lookup/application-types/:id`, `PATCH lookup/test-types/:id` with `ParseIntPipe`; `pnpm typecheck` 4/4 + `pnpm build` 3/3 green — web route table unchanged).
**Decisions made:**
- **Single-field partial PATCH via optional DTO + whitelist immutability** — each PATCH accepts one field per blur (11.2 autosave); DTOs optional with `ValidateIf(value !== undefined)` so `{classFees:25}` passes but `{className:"foo"}` 400s via `forbidNonWhitelisted`; empty `{}` rejected in service with 400 (class-validator would pass an all-optional DTO).
- **Fee persistence `toFixed(2)` → decimal string** — lookup entities store `decimal(10,2)` strings; service converts validated `number` via `toFixed(2)` before `save` (detain-license precedent).
- **No transaction, single-row `save`** — each PATCH touches one lookup row only, no cross-table write and no `PaidFees` snapshot (#28 — past rows unaffected; service header "writes affect only the lookup row" for REVIEW).
- **Validation ranges** `minimumAllowedAge 1–120`, `defaultValidityLength 1–50`, fees `0–99999999.99` reuse detain caps and seed ranges (16/18/21, 5/10yrs) without hardcoding current config values.
**Deviations from plan:** none — build-plan §11.1 single-field partial updates implemented exactly (the 3 `PATCH /lookup/.../:id` routes); no UI code touched (logic-only per AGENTS.md §3.1).
**Known issues / follow-ups:**
- **No API boot + no smoke (session pattern)** — `pnpm typecheck` 4/4 + `pnpm build` 3/3 green (web unchanged); the 3 PATCH endpoints, 404/400 guards, `ParseIntPipe`, whitelist rejection of `className`/`applicationTypeTitle`/`testTypeTitle`, `toFixed(2)`, and #28 isolation unverified at runtime.
- **REVIEWs still owed (user's call): 11.1 `[LOGIC]` now fronts the queue** (cross-refs: 3 PATCH routes under `@Controller('lookup')`, DTO validators + whitelist, empty-body 400, `toFixed(2)`, `findById` 404, #28 isolation, #31 no role gate, comment/hex policy — zero comments in controller/DTOs), then backlog 10.2+10.1, 9.2+9.1, 8.1/8.2, 7.1/7.2, 6.1, 5.1, 5.2, 0.B.2/0.C.1/1.1/4.1/4.2.
- Carried: 6.1-vs-7.1 + double-detain concurrency holes (no partial unique index backstops); pg deprecation warning; Completed-after-refresh banner gap; Roles column data source; citizen-options 1000-window; TestType descriptions provisional; ui-rules.md neutral-tint pill sync (now four precedents — Expired/Inactive/Released + 10.2's Expired/Inactive); new tabs pill styling vs `ui-registry` `TabbedDetailView` line-variant default. Queued 7.1 renew/replace expiry patch still awaits a user directive.
**Start next session with:** Per AGENTS.md §3.1 decide with the user: **REVIEW pass on 11.1 `[LOGIC]`** (cross-refs above), or go straight to **11.2 — Configuration `[UI]`** (ARCHITECT first: `/settings/configuration` page per `display system tables for lookups.jpg` — three cards: Application types `ID/Title read-only/Fee $` (6 rows), Test types `ID/Test+description read-only/Fee $` (3 rows), License classes full-width `ID/Class read-only/Min age/Validity(yrs)/Fee $` (7 rows); autosave per-field on blur/`Enter` with inline saved indicator, no page Save; `configurationKeys` + `useLicenseClasses`/`useUpdateLicenseClass`/`useApplicationTypes`/`useUpdateApplicationType`/`useTestTypes`/`useUpdateTestType` hooks). Also queued: the 7.1 renew/replace expiry patch.

### Session 25 — 2026-08-21
**Completed:** **10.2 — Driver & License History `[UI]`** (full vertical slice per the two approved descriptive prompts — ARCHITECT + go. `driverKeys` factory (`all/lists/list/search/details/summary/localLicenses/internationalLicenses/testLog`); `drivers.service` — `getDrivers`/`searchDrivers`/`getDriverSummary`/`getDriverLocalLicenses`/`getDriverInternationalLicenses`/`getDriverTestLog` via `apiClient`; 6 hooks — `useDrivers`/`useDriverSearch` (enabled on trimmed search)/`useDriverSummary`/`useDriverLocalLicenses`/`useDriverInternationalLicenses`/`useDriverTestLog` with `placeholderData`; `DriverLookupCard` — white `rounded-xl border bg-card shadow-sm` card, `Search` leading-icon input + primary `Search` button, plain controlled state + `defaultValue` sync; `DriverDirectoryTable` — `DataTable` `showSearch={false}` + `header` with `Showing results for "term" · Clear`, paging `PAGE_SIZE=10`, switches `useDrivers`↔`useDriverSearch` via `isSearching`, `useEffect` page-reset on term; columns `DRV-N` mono, Name+email stacked, `N-…` mono, `N active / M total`, `In Good Standing`/`Has Detained License` pills, `View History` outline → `/drivers/:id`; `DriverProfileSummaryCard` — avatar `bg-primary/10` initials + name + mono national header, 4-col KV grid `DoB/Email | Gender/Address | Phone/DriverID | Country/Since` collapsing `1→2→4` cols + skeleton; `LocalLicenseHistoryTable` — `LIC-N`/Class/Reason/Issued/Expires/Fees/Status three-pill reuse (7.2); `InternationalLicenseHistoryTable` — `INT-N`/Based On `LIC-N`/Issued/Expires/Status from dates, drops redundant Driver column; `DriverTestLogTable` — `TEST-N`/Stage/Date/Result Passed-Failed/Fees/`L-N`/Notes truncated; `DriversPage` + `DriversDetailPage` — H1 + subtitle, reusable lookup (detail → `router.push(/drivers?search=term)`), pill `Tabs` (shadcn Tabs generated `radix-nova`, restyled `rounded-full` active `bg-accent` vs inactive `bg-muted` with live counts, `bg-transparent p-0` list), bottom `Back to all drivers` outline; routes `app/(protected)/drivers/page.tsx` + `[id]/page.tsx` async reading `searchParams` for `?search=` pre-fill; `components/ui/tabs.tsx` generated via `shadcn add tabs`.)
**Decisions made:**
- **Search = server-side `GET /drivers/search`** — directory switches feed, `Clear` restores `GET /drivers`; detail lookup pushes to `/drivers?search=term`; both routes use async `searchParams` prop for `?search=` pre-fill (avoids `useSearchParams` Suspense bailout).
- **Lookup card = plain controlled input** (not RHF/zod — search precedent; TopBar/DataTable filter pattern) — `useEffect` syncs internal `value` when `defaultValue` changes (`?search=` navigation).
- **Directory `page` resets on `searchTerm` change** — `useEffect(() => setPage(1), [searchTerm])` prevents stale page N on a narrower result set.
- **Pill tabs = shadcn `Tabs` + className-only restyle** — generated `Tabs`/`TabsList`/`TabsTrigger`/`TabsContent`; `TabsList` `bg-transparent p-0`, triggers `rounded-full` active `bg-accent text-accent-foreground font-bold shadow-sm` vs inactive `bg-muted text-muted-foreground`; 4 independent queries at page level for live counts (build-plan's "four small queries").
- **History tables `showSearch={false}`, `total=rows.length`** — endpoints return arrays, no pagination; pill/date patterns reuse 7.2/8.2 (three-pill Status, en-GB `T00:00:00` parse for DATE, `new Date(iso)` for timestamptz, `INT-N`/`LIC-N`/`TEST-N`/`L-N` mono, `todayIso()` string compare for international `Expired`).
**Deviations from plan:** build-plan § 10.2 "same shape as Feature 8, filtered to this driver" — dropped redundant Driver column from the international tab (page is the driver; matches how local tab drops Driver/National from 7.2 shape — noted for REVIEW); otherwise no substance.
**Known issues / follow-ups:**
- **No API boot + no smoke (session pattern)** — `pnpm typecheck` 4/4, `pnpm build` 3/3 green (`ƒ /drivers` 2.11 kB, `ƒ /drivers/[id]` 8.49 kB), hex/comment grep clean (0 hits). The 6 driver endpoints, LIKE predicates, `hasDetainedLicense` EXISTS flag, `?search=` wiring, pill counts, and `requireDriver` 404 guard unverified at runtime.
- **REVIEWs still owed (user's call): 10.2 + 10.1 as a pair now fronts the queue** (cross-refs: server-side search wiring + page-reset + `?search=` pre-fill, `hasDetainedLicense` parity, person-link `test.id IS NOT NULL`, pill token + new tabs primitive, `toDto` gates, `#9`/`#11`, comment/hex policy, page thinness, feature-boundary), then backlog 9.2+9.1, 8.1/8.2, 7.1/7.2, 6.1, 5.1, 5.2, 0.B.2/0.C.1/1.1/4.1/4.2.
- Carried: 6.1-vs-7.1 + double-detain concurrency holes (no partial unique index backstops); pg deprecation warning; Completed-after-refresh banner gap; Roles column data source; citizen-options 1000-window; TestType descriptions provisional; ui-rules.md neutral-tint pill sync (now four precedents — Expired/Inactive/Released + 10.2's Expired/Inactive); new tabs pill styling vs `ui-registry` `TabbedDetailView` line-variant default. Queued 7.1 renew/replace expiry patch still awaits a user directive.
**Start next session with:** Per AGENTS.md § 3.1 decide with the user: **REVIEW pass on 10.2 `[UI]` + 10.1 `[LOGIC]` together** (cross-refs above), or go straight to **11.1 — Configuration `[LOGIC]`** (ARCHITECT first: `PATCH /lookup/license-classes/:id`, `/application-types/:id`, `/test-types/:id` single-field partial updates). Also queued: the 7.1 renew/replace expiry patch ((a) renewal 409 when `expirationDate > today + 6 months`, (b) replacement preserves OLD `ExpirationDate`, (c) 409 "renew it instead" on expired replacement).

### Session 24 — 2026-08-20
**Completed:** **10.1 — Driver & License History `[LOGIC]`** (full vertical slice: `packages/shared/src/dtos/driver.dto.ts` — `DriverDirectoryRowDto` / `DriverSummaryDto` / `DriverTestLogEntryDto` + barrel export; `modules/drivers/` — new `repositories/drivers.repository.ts`, `drivers.service.ts` extended with 6 read methods (findOrCreateByPersonId preserved), new `drivers.controller.ts` (6 GET routes), module rewired with 4 foreign entities + repository/controller). ARCHITECT pass first (user approved the feature + confirmed the two UI specs map 1:1 to the 10.1 endpoints before build); user approved skip of the backlog REVIEW queue and build 10.1 directly.
**Decisions made:**
- **Directory builder = correlated COUNT subqueries + open-detention EXISTS via `addSelect`/`getRawAndEntities`** (7.1 register precedent) — no new relations on the `Driver` entity needed; `totalLicenseCount`/`activeLicenseCount`/`hasDetainedLicense` ride raw/entity zip, service projects to the flat DTO.
- **Cross-module reads use generic TypeORM repositories, not foreign repository classes** — `License`, `InternationalLicense`, `TestAppointment` registered in `DriversModule.forFeature` and read via `@InjectRepository` in `DriversRepository`; no other module's repository is imported (module-boundary rule holds; repo classes keep single ownership). Dashboard (12.1) may copy this aggregation pattern.
- **Local-license history reuses `LicenseRegisterRowDto`** (7.2 shape including `isDetained` → the three-pill Status renders without new fields); **international history reuses Feature 8's `InternationalLicenseDto`** (build-plan § 10.2 "same shape as Feature 8, filtered to this driver").
- **Test log = recorded `Tests` rows only** (`test.id IS NOT NULL`), newest first, each entry carrying appointment metadata (stage title, ISO date, `isLocked`, `paidFees`, application + LLA ids). Tests reach the driver through appointment → LLA → Application → applicant Person = the driver's own person (no driver relation needed on Person).
- **All five `/:id/...` endpoints share `requireDriver()` 404 guard**; `GET /drivers` + `GET /drivers/search` both paginated (every-register precedent); search = one case-insensitive LIKE across driver id (`CAST(driver.id AS varchar)`), national number, first/last/full concatenated name. Route `search` literal precedes the `:id` routes with no conflict; a third `@Controller('drivers')` (alongside the international module's eligible controller) coexists fine.
**Deviations from plan:** none of substance — test log returns recorded Tests rows with appointment metadata exactly as planned ("every Tests row ... including locked/appointment metadata"); directory + search paginated (register precedent); DTOs flat and shared (#9).
**Known issues / follow-ups:**
- **No API boot + no smoke (session pattern)** — `pnpm typecheck` 4/4 + `pnpm build` 3/3 green (web route table unchanged — no UI touched). `@repo/shared` must be rebuilt before API typecheck sees new DTOs (`pnpm --filter @repo/shared build`). All 6 routes, the search predicates, EXISTS/count raw aliases, and the 404 guard unverified at runtime.
- Typecheck catch mid-build: `raw[index]` on typed `Record<string, unknown>[]` errors under `noUncheckedIndexedAccess` (TS2532) — fixed with `const row = raw[index]!` inside the mapper (the 7.1 raw zips never hit this because their `raw` comes untyped from `getRawAndEntities()`).
- **REVIEWs still owed (user's call): 10.1 `[LOGIC]` is now the front of the queue** (cross-refs: correlated-subquery counts vs join+group semantics, EXISTS flag parity with 7.1's register, test-log person-link path + `test.id IS NOT NULL`, module boundary — foreign entities registered for reads only, route ordering, `toDto` gates, #9/#11, comment policy), then the backlog (9.2 + 9.1, 8.1/8.2, 7.1/7.2, 6.1, 5.1, 5.2, 0.B.2/0.C.1/1.1/4.1/4.2).
- Carried: flagged concurrency holes (6.1-vs-7.1 same-class; double-detain); pg deprecation warning; Completed-after-refresh banner gap; Roles column data source; citizen-options 1000-window; TestType descriptions provisional; ui-rules.md neutral-tint pill sync (three precedents now — Expired/Inactive/Released). Queued 7.1 renew/replace expiry patch still awaits a user directive.
**Start next session with:** Per AGENTS.md § 3.1 the REVIEW queue precedes the next feature — decide with the user: **REVIEW pass on 10.1 `[LOGIC]`** (cross-refs above), or go straight to **10.2 — Driver & License History `[UI]`** (ARCHITECT first: `/drivers` page — Driver Lookup card (search input + primary "Search" button → `GET /drivers/search`) + Registered Drivers DataTable (Driver ID `DRV-N` mono, Name+email stacked, National Number `N-...`, Licenses `[N] active / [M] total`, Status pill In Good Standing `bg-success/10` / Has Detained License `bg-destructive/10`, View History outline button) per the user's two approved descriptive prompts; `/drivers/[id]` page — profile summary card (initials avatar, 4-col key-value grid: DoB/Email, Gender/Address, Phone/DriverID, Country/Since per the user's prompt layout), pill tab bar Local Licenses (n) / International (n) / Test Log (n) with live counts, local table (License/Class/Issue Reason/Issued/Expires/Fees/Status), "Back to all drivers" link; `driverKeys` factory + service + `useDrivers` / `useDriverSearch` / `useDriverSummary` / `useDriverLocalLicenses` / `useDriverInternationalLicenses` / `useDriverTestLog` hooks). Also queued: the 7.1 renew/replace expiry patch.

### Session 23 — 2026-08-19
**Completed:** **9.2 — Detain & Release System `[UI]`** (full vertical slice: `/detain-release` route + page, `features/detain-release/` — keys factory, web request DTO, service, 4 hooks; `DetainLicenseFormCard`; `DetentionRegisterTable`; `ReleaseDetentionModal`). Built per the user's exact descriptive prompts + build-plan; no API boot or smoke (user directive).
**Decisions made:**
- **Two-column page per the ui-rules-registered grid** — `grid-cols-1 gap-6 lg:grid-cols-[360px_1fr]` (form card left / register right; the Prompt's "~1/3 — ~2/3" is exactly this rule's existing row for this page — no new layout token).
- **Detain stays an inline form, not a modal** (build-plan § 9.2 "not a modal-driven flow" + ui-registry `DetainLicenseFormCard`) — the typed two-field submit (pick license + enter fine + explicit "Detain license" shield button) is the confirmation; no second Confirm modal (ui-rules' "detain a license" row sits in the Modals/destructive section and the 9.2 plan overrides it for this flow).
- **Register table = shared DataTable with `header` + `showSearch={false}`** (Session 19 props): title "Detention register" + subtitle inside the card; 8 columns per spec — Detain `#N` (prompt-literal prefix, mono bold), Driver name + mono national number stacked (7.2 cell), License `LIC-{licenseId}` (7.2/8.2 license-ID convention), Detained `[DD Mon YYYY]` en-GB parse of the ISO timestamptz directly (`new Date(iso)` — NOT the `T00:00:00` trick used for DATE columns), Fine `$` tabular-nums, Total due `$` bold, Status pill, Actions.
- **Status = TWO pills** (the register includes released rows — 9.1 audit-trail decision): Detained `bg-destructive/10 text-destructive` (soft red per prompt, 7.2 Detained precedent), Released `bg-neutral-tint text-neutral-tint-foreground` (gray — Inactive/Expired precedent, archived semantics distinct from red). Release button = outline `Unlock` + "Release" label (prompt: not icon-only), disabled on released rows with title "This license was already released." + `disabled:cursor-not-allowed [&:disabled]:pointer-events-auto` (7.2 disabled-why rule).
- **Both mutations invalidate `lists()` AND `eligibleLicenses()`** — detain removes the license from the picker; **release makes it eligible again** (license returns to good standing → back in the "Select active license" feed immediately).
- **Release fee displayed live, never hardcoded (invariant #28)** — `useApplicationTypes` + `ApplicationType.RELEASE_DETAINED_LICENSE` in BOTH the form's helper text ("Release collects the fine plus a $X release application fee.", `$—` while pending) and the modal's breakdown; modal restates License/Driver/Fine/live release fee/Total due with "Confirm Release · $X" button (8.2 fee-labeled-button precedent).
- **First fee INPUT in the codebase** (all prior features only displayed fees) — zod refines mirror the backend DTO: `.min(0)`, `.max(99999999.99)`, 2-decimal via `Math.round(v*100)/100 === v` (safe on doubles, unlike `.multipleOf(0.01)`), `number({ error: ... })` — **zod v4 syntax** (v3's `invalid_type_error` is rejected by tsc in this repo).
- **Fine Fees input = `InputGroup` with `InputGroupAddon "$"` + `InputGroupInput`** (class `h-10` overrides the primitive's `h-8` via twMerge; `aria-invalid` on the control drives the group's destructive border).
- Eligible-license Select options rendered `"DriverName · LIC-N"`; picker feed = full 1000-window ride on `GET /detain-release/active-licenses` (citizen-options precedent, 5-min staleTime); feed pending → trigger disabled + "Loading licenses…", error → inline Try again link (4.2 modal precedent).
**Deviations from plan:** none of substance — option format (driver-first, license suffix) chosen from the prompt's Driver-centric rows; subtitle added to the register card header (8.2 register precedent); no notes field anywhere (9.1 has none).
**Known issues / follow-ups:**
- **No API boot + no smoke (session pattern)** — `pnpm typecheck` 4/4, `pnpm build` 3/3 green (route table adds `/detain-release`, 7.76 kB static; no other route touched). Comment grep clean (zero comments in all new files — policy intact); hex grep clean (tokens only). The 4 routes, gates, live fee reads, and invalidation graph unverified at runtime.
- **REVIEWs still owed (user's call): 9.2 + 9.1 (pair — the feature is now complete; scrutinize the first-ever bidirectional forwardRef module/provider cycle and the double-detain concurrency hole), then 8.1/8.2, 7.1/7.2, 6.1, 5.1, 5.2, backlog 0.B.2/0.C.1/1.1/4.1/4.2.**
- Carried: flagged concurrency holes (6.1-vs-7.1 same-class; double-detain); pg deprecation warning; Completed-after-refresh banner gap; Roles column data source; citizen-options 1000-window; TestType descriptions provisional; ui-rules.md sync for neutral-tint Expired/Inactive/Released pills (now three shipped precedents). The queued 7.1 renew/replace expiry patch still awaits a user directive.
**Start next session with:** Per AGENTS.md § 3.1 the REVIEW queue precedes the next feature — decide with the user: **REVIEW pass on 9.2 `[UI]` + 9.1 `[LOGIC]` together** (cross-refs: forwardRef minimality [one-side vs bidirectional], double-detain race, #27 release-only path, #28 live-fee display + display-only totalDue, #29 session user both writes, #32 guard intact, DataTable header/showSearch reuse, pill token mapping, comment/hex policy), or go straight to **10.1 — Driver & License History `[LOGIC]`** (ARCHITECT first: DriversRepository findAll/search, 4 driver-summary endpoints per build-plan § 10.1). Also queued: the 7.1 renew/replace expiry patch.

### Session 22 — 2026-08-19
**Completed:** **9.1 — Detain & Release System `[LOGIC]`** (full vertical slice on the Session 18 head start — table + entity + `hasOpenDetention` already lived). ARCHITECT pass first (user approved forwardRef cycle resolution + plan-literal eligibility).
**Decisions made:**
- **First module cycle in the codebase, resolved with bidirectional `forwardRef` (user-approved).** `LicensesModule ↔ DetainReleaseModule`: the #32 guard (licenses → detain) meets the 9.1 eligible read + detain write (detain → licenses). Both module `imports` use `forwardRef(() => ...)` AND both service constructors use `@Inject(forwardRef(() => ...))` (NestJS docs pattern for provider-level cycles). Session 16's "importing foreign entities into licenses.service was rejected" precedent kept 7.1's guard untouched; the reverse option (relocating the guard to the licenses domain) was rejected in favor of the cycle.
- **Eligibility plan-literal (user-approved):** `IsActive = true` + no open detention, **no expiry gate** — deliberate departure from Session 20's international gate; detention is a violation-based physical confiscation (an officer can confiscate an expired card too). The picker feed and the server-side detain guard both key on this.
- **Register = every detention, released rows included** (audit trail, 7.2 include-deactivated precedent), paginated, newest first, no search. `totalDue = FineFees + ReleaseDetainedLicense.ApplicationFees` computed per row server-side from the **live** lookup fee — never persisted (the release fee is only snapshotted at release time, build-plan § 9.1).
- **All detain guards in ONE transaction** (8.1 precedent): 404 license (404 for missing, 409 for inactive — via new manager-parameter `LicensesService.findLicenseOnManager`), 409 open detention, then insert with `fineFees.toFixed(2)` (decimal column typed string), `createdByUserId` (session, #29). Release fee resolved pre-tx so a missing type can't leave a committed detention behind a 404.
- **Release = ReleaseApplication (Completed, 7.1 create-in-transaction precedent) + guarded `UPDATE ... WHERE isReleased = false`** (`affected !== 1` → 409, the concurrent-double-release race backstop), setting `releaseDate`, `releasedByUserId` (session, #29), `releaseApplicationId` — invariant #27: no other path may flip `IsReleased`. Applicant = the license's driver person (from the joined detention row).
- **Eligible read lives in the licenses domain** (new `LicensesRepository.findActiveLicensesWithoutOpenDetention` NOT EXISTS predicate, exposed via `LicensesService.findEligibleForDetention`); DTO mapping in the detain-release service (8.1 international precedent). Paginated route on `GET /detain-release/active-licenses` rides the page-1/1000 client pattern later.
- **Shared DTOs:** `DetentionRegisterRowDto` (id, licenseId, driverId, driverName, nationalNumber, detainDate ISO, fineFees string, isReleased, releaseDate, totalDue) + `EligibleLicenseForDetentionDto` (licenseId, driverName, nationalNumber, className). Backend `DetainLicenseRequestDto` = `{ licenseId @IsInt @IsPositive, fineFees @IsNumber(maxDecimalPlaces 2) @Min(0) @Max(99999999.99) }`.
**Deviations from plan:** none of substance — route `:id` on release = `DetainID` (the register lists detentions); release application created Completed (7.1 precedent, plan's "processes it" is one transaction); register paginated (every-register precedent vs plan-literal "every detention").
**Known issues / follow-ups:**
- **No API boot + no smoke (user directive)** — `pnpm typecheck` 4/4 + `pnpm build` 3/3 green (web route table unchanged — no UI touched). The 4 new routes, all 409/404 gates, the forwardRef module graph, and the Session 18 MCP table unverified at runtime.
- **Never-before-seen patterns the 9.1 REVIEW must scrutinize:** the bidirectional forwardRef module + provider cycle (is one-side forwardRef sufficient? are both @Inject(forwardRef) needed or only one?) — and the **double-detain concurrency hole**: the open-detention guard is an in-tx read only, no partial unique index backstop (same deferred class as the flagged 6.1-vs-7.1 hole); a license CAN be re-detained after release, so a plain unique on LicenseID is wrong.
- Comment policy catch mid-build: explanatory comments had slipped into the two **module files** (non-exempt) — removed before close; final grep across controller/module/DTO/shared clean.
**Start next session with:** Per AGENTS.md § 3.1 the REVIEW queue precedes the next feature — decide with the user: **REVIEW pass on 9.1 `[LOGIC]`** (invariant cross-refs: #27 release-only path, #28 fee snapshot + display-only totalDue, #29 session user on both writes, #32 guard intact after the forwardRef refactor, #9 single shared DTOs, #11 toDto gates, tx atomicity on both flows, race guards, the forwardRef cycle itself, comment policy) — or go straight to **9.2 — Detain & Release System `[UI]`** (ARCHITECT first: `/detain-release` two-column page — left `DetainLicenseFormCard` (License select from `GET /detain-release/active-licenses`, Fine Fees ($) input, "Detain license" shield button, helper text reading the release fee live from `useApplicationTypes`), right "Detention register" `DataTable` (Detain `#N`, Driver, License `#N`, Detained, Fine, Total due, Status `Detained` pill, Release action), Release confirmation modal restating the total, hooks `useEligibleLicensesForDetention` / `useDetentionRegister` / `useDetainLicense` / `useReleaseLicense` + `detainReleaseKeys`). Also queued: the 7.1 renew/replace expiry patch.

### Session 21 — 2026-08-19
**Completed:** **8.2 — International License Service `[UI]`** (full vertical slice: `/applications/international` route + page, `features/international-licenses/` — keys factory, web request DTO, service, 3 hooks; `InternationalLicensesTable`; `IssueInternationalLicenseModal`). Built per the user's exact descriptive prompts + build-plan; no API boot or smoke (user directive).
**Decisions made:**
- **Status derived from dates, not `isActive`** — Active when `expirationDate >= today` (local YYYY-MM-DD string compare), Expired otherwise; nothing in the 8.1 write path ever flips `isActive`, so the column can't drive the pill (Session 20 "Active/Expired registers from dates" note). UI colors per user spec: Active `bg-success/10 text-success`, **Expired = soft gray** `bg-neutral-tint text-neutral-tint-foreground` (deliberate follow of the prompt's "soft gray badge", matching the 7.2 Inactive treatment; ui-rules' blanket Expired→destructive listing stays flagged for a ui-rules sync).
- **Modal fee comes from lookup, never hardcoded** — `NewInternationalLicense` application type via `useApplicationTypes` (invariant #28); shown in the verification box's "Application Fee" row AND the confirm button label ("Issue License · ${fee}", "—" while pending). Validity row = the fixed "1 year from issue date" copy (library-docs § 8 face).
- **Verification panel renders only when a driver is selected** (`form.watch("driverId")` truthy) — `bg-success-tint` (#F0FDF4 spec) bordered panel with circular green check + bold green "Verified: active Class 3 (Car) local license on file." (spec copy); confirm button `disabled` until then ("Disabled state when unverified" — options only ever contain eligible drivers, so selection ⇒ verified).
- **Driver picker = shared `SearchableCombobox<InternationalEligibleDriverDto>`** (ui-registry reuse, not a re-implemented dropdown), RHF `Controller` mirroring `NewLocalApplicationModal`'s citizen picker; feed rides `GET /drivers/eligible-for-international` page-1/pageSize-1000 (citizen-options precedent; 5-min staleTime). The owner keeps only `driverId` (combobox pattern).
- **Hook contracts:** `useInternationalLicenses(params)` with `placeholderData` (7.2 pattern), `useEligibleInternationalDrivers()` full feed, `useIssueInternationalLicense()` invalidates `internationalLicensesKeys.lists()` only — the eligible feed is unchanged by issuance (the driver keeps their Car license), and the 409 duplicate guard surfaces verbatim in the modal's alert box (dialog stays open). No success banner — the register self-updates (7.2 precedent).
- **Table = shared DataTable with `header` + `showSearch={false}`** (Session 19 props) — "Issued International Licenses" section title + "All international licenses on record, newest first." subtitle inside the card; six columns per spec (License `INT-{id}` mono bold, Driver name+national number stacked, Based On `LIC-{id}` mono, Issued/Expires `[Mon DD, YYYY]` en-US tabular-nums, Status pill), paginated register precedent.
- **Page header per people-page pattern** — H1 "International Licenses" + muted subtitle (spec copy), right-aligned primary `New International License` (`BadgePlus` icon, h-10).
**Deviations from plan:** modal driver picker uses the shared combobox instead of a plain `Select` (registry mandate — combobox is the registered driver-picker pattern); eligible feed shipped as a full 1000-window ride on the paginated endpoint (plan's "eligible drivers only" picker needs the full set — citizen-options precedent); Expired pill gray per user's exact prompt (ui-rules row still lists Expired→destructive — flagged below).
**Known issues / follow-ups:**
- **No API boot + no smoke (session pattern)** — `pnpm typecheck` 4/4, `pnpm build` 3/3 green (route table adds `/applications/international`, 4.42 kB static; no other route touched). Comment grep clean (zero comments in all new files — policy intact); hex grep clean (tokens only).
- **Eligible picker includes drivers who already hold a valid international license** — the eligible endpoint keys off the Car license only, so a second issuance attempt for the same driver 409s in the modal alert (server-enforced, 8.1 guard; UI cannot pre-filter — no such field). Consider for Feature 10/review.
- **REVIEWs still owed (user's call): 8.1 + 8.2 (this feature completes — REVIEW the pair), then 7.1, 7.2, 6.1, 5.1, 5.2, backlog 0.B.2/0.C.1/1.1/4.1/4.2.**
- Carried: flagged concurrency hole (6.1 vs 7.1 same-class); pg deprecation warning; Completed-after-refresh banner gap; Roles column data source; citizen-options 1000-window; TestType descriptions provisional. The queued 7.1 renew/replace expiry patch (renewal window, replacement preserves expiry, 409 on expired replacement) still awaits a user directive.
**Start next session with:** Per AGENTS.md § 3.1 the REVIEW queue precedes the next feature — decide with the user: **REVIEW pass on 8.1 `[LOGIC]` + 8.2 `[UI]` together** (cross-refs: #24 eligibility + expiry gate, #28 fee snapshot + live modal fee, #29 session user, duplicate guard, tx atomicity, manager-parameter read, comment policy, DataTable header/showSearch reuse, combobox reuse, pill token mapping), or go straight to **9.1 — Detain & Release System `[LOGIC]`** (MCP migration `detained_licenses` already exists from 7.1; build the module's service/controllers/DTOs on the Session 18 head start). Also queued: the 7.1 renew/replace expiry patch.

### Session 20 — 2026-08-19
**Completed:** **8.1 — International License Service `[LOGIC]`** (full vertical slice: MCP migration, shared DTOs, `modules/international-licenses/` entity/repository/service/DTO/two controllers/module, licenses-domain eligibility read, lookup class-by-title finder, app wiring; no UI code touched). ARCHITECT pass first (user answered 2 gate-scope questions + queued a 7.1 follow-up).
**Decisions made:**
- **Eligibility gate = active AND unexpired** (user-approved recommendation over the invariant #24 literal) — nothing auto-deactivates a license at expiry, so `IsActive = true` alone would let a long-expired, never-renewed license back an international document. No detention check (spec never mentions it for international — #32 stays renew/replace-scoped).
- **Duplicate guard: 409** when the driver already holds an international license with `ExpirationDate >= today` (user-approved) — one valid international document per driver at a time (nothing ever sets the DB `IsActive` false; Active/Expired registers from dates).
- **Car class resolved by exact `ClassName`** via new `LookupService.findLicenseClassByTitle('Ordinary Driving License (Car)')` + `LicenseClassesRepository.findByTitle` — invariant #24 names the class by wording and Feature 11 edits age/validity/fees but never names; no hardcoded class id; 404 fail-loud if the row vanished.
- **In-tx eligibility read = new manager-parameter `LicensesService.findActiveUnexpiredLicenseOnManager(manager, driverId, licenseClassId)`** — the guard sees the transaction's own uncommitted state (Session 16 manager-parameter pattern); the licenses module keeps ownership of `License` readers, so the international module never registers foreign entities. `relations: { driver: { person: true } }` supplies the applicant person id for the application row.
- **Eligible-drivers read stays in the licenses domain** — `LicensesRepository.findActiveCarLicenses` (paginated, active+unexpired, joined) exposed via `LicensesService`; the international service maps rows to `InternationalEligibleDriverDto` (driverId, driverName, nationalNumber, localLicenseId). `LicensesModule` now `exports: [LicensesService]` (first export added).
- **Issuance = ONE transaction** with gate order: 404 Car-class config → 404 New International application type → **tx**: 400 no active+unexpired Car license (covers nonexistent driver) → 409 valid existing international → Completed application via `appsService.createInTransaction` (fee snapshot #28) → insert (`issueDate` today, `expirationDate` +1yr fixed rule per library-docs § 8, `IsActive` true, session user #29).
- **Request = `{ driverId }` only** — `POST /international-licenses`; no notes (the table has no Notes column). `GET /international-licenses` + `GET /drivers/eligible-for-international` both paginated (register precedent); eligible controller rides the foreign-prefixed route under `@Controller('drivers')` (Session 18 two-controllers + foreign-prefix precedent).
- **QUEUED FOLLOW-UP (user decision, domain consultation):** 7.1 `renewOrReplace` guards — (a) renewal window: reject with 409 when `expirationDate > today + 6 months` (else a fresh license buys 10 extra years for a $10 renewal); (b) replacement must preserve the OLD license's `ExpirationDate` (physical card swap, not validity re-issue — currently `renewOrReplace` mints a fresh full term for replacements too); (c) replacement of an already-expired license → 409 "renew it instead".
**Deviations from plan:** expiry requirement + duplicate guard beyond the plan-literal walkthrough (both user-approved); eligible endpoint paginated (every-register precedent); build-plan's shared eligible-driver feed now exists as 3 endpoints instead of the planned count. MCP migration for the table per Session 18 precedent (no TypeORM migration file).
**Known issues / follow-ups:**
- **No API boot + no smoke (session pattern)** — `pnpm typecheck` 4/4 + `pnpm build` 3/3 green (web route table unchanged). New routes, gates, 400/409 paths, and the MCP migration unverified at runtime.
- Comment policy catch: controller comments slipped into `eligible-international-drivers.controller.ts` during the build — caught by self-grep and removed before close.
- **Queued 7.1 patch** (above — renew/replace expiry semantics; user chose to queue it).
- **REVIEWs still owed (user's call): 8.1 `[LOGIC]` itself, then 7.1, 7.2, 6.1, 5.1, 5.2, backlog 0.B.2/0.C.1/1.1/4.1/4.2.**
- Carried: flagged concurrency hole (6.1 issuance vs 7.1 renewal same-class, no partial-unique-index backstop); pg deprecation warning; Completed-after-refresh banner gap (register DTO exists — closable in 6.2); Roles column data source; citizen-options 1000-window; TestType descriptions provisional.
**Start next session with:** Per AGENTS.md § 3.1 the REVIEW queue precedes the next feature — decide with the user: **REVIEW pass on 8.1 `[LOGIC]`** (cross-refs: #24 eligibility + expiry gate, #28 fee snapshot, #29 session user, duplicate guard, tx atomicity, manager-parameter read, comment policy, module boundary: international imports licenses service only), or go straight to **8.2 — International License Service `[UI]`** (ARCHITECT first: `/applications/international` page — table License/Driver/Based On (local license ID)/Issued/Expires/Status(Active/Expired from dates); `IssueInternationalLicenseModal` with eligible-driver picker from `GET /drivers/eligible-for-international` + fee-labeled confirm; `internationalLicenseKeys` + service + `useInternationalLicenses`/`useIssueInternationalLicense`/`useEligibleDrivers` hooks). Also queued: the 7.1 renew/replace expiry patch.

### Session 19 — 2026-08-19
**Completed:** **7.2 — License Renewal & Replacement `[UI]`** (full vertical slice: `/applications/renewals` route + page, `features/renewals-replacements/` — keys factory, 2 web request DTOs, service, 3 hooks; `LicenseRegisterTable` with all 8 spec columns; `RenewLicenseModal` + shared `ReplaceLicenseModal`; DataTable extension).
**Decisions made:**
- **DataTable gained optional `header` + `showSearch` props (backward-compatible defaults)** — the register card needs a "Local license register" section header inside the card AND no filter input (Session 18 decision: register is paginated, no search; deviates from the plan-literal DataTable-with-filter shape).
- **Status = THREE pills, not two:** Active `bg-success/10 text-success` / Detained `bg-destructive/10 text-destructive` / Inactive `bg-neutral-tint text-neutral-tint-foreground` — the register includes deactivated rows (audit trail, 10.1 reuse); gray chosen to visually separate archive from detained (deliberate deviation from ui-rules' blanket "Inactive→destructive" row; color + label pairing keeps a11y — flag for ui-rules sync).
- **Disabled actions reveal the "why":** native `disabled` + `title` + `disabled:cursor-not-allowed [&:disabled]:pointer-events-auto` — the base button's `pointer-events-none` would deaden hover so the title/cursor never surface (violating ui-rules' "never guess why"); re-enabling pointer events on the disabled state makes them work while the button stays non-activatable. Titles: detained → "Release this license before renewing or replacing it" (invariant #32 mirror); inactive → "This license is no longer active." (server 409s anyway).
- **One `ReplaceLicenseModal` serves BOTH reasons** via `reason: 'damaged'|'lost'` + a `REPLACE_META` record (title/copy/placeholder/icon FileWarning vs FileX) — no duplicated modal pair (mirrors 6.2's per-modal-file convention while keeping the 7.x pair DRY).
- **Modal description = load-bearing consequence copy:** names the deactivated `LIC-{id}` (mono emphasis) + BOTH fees live from lookup — application fee matched by `ApplicationType` title via `useApplicationTypes`, license fee via `useLicenseClasses` by row classId (invariant #28; "—" while pending). Notes textarea ≤500, placeholder = issue-reason copy family ("Renewal of license." / "Replacement for damaged license." / "Replacement for lost license." — the 6.2 "First time issuance." precedent).
- **Reason labels** First Time / Renewed / Damaged / Lost (IssueReason 1-4); dates `[DD Mon YYYY]` via en-GB `Intl.DateTimeFormat` with local-midnight parse — **no date-fns added to web** (not a dependency there).
- **Mutations bind the license id at hook call** (`useRenewLicense(license.id)`), invalidate `renewalsReplacementKeys.lists()` only on success — the old row flips to Inactive and the new row appears in the refetch, so no success banner is needed (6.2's banner existed for a detail-page CTA; the register self-updates).
- Single `pendingAction { license, action }` state in the table — exactly one modal open at a time.
**Deviations from plan:** none of substance — plan-literal "Status (Active/Detained pill)" extended with the Inactive case (register content demands it); hooks invalidate only the register lists (no `driversKeys` until Feature 10 — carried 6.2 precedent); notes field ships per 7.1's user amendment.
**Known issues / follow-ups:**
- **No API boot + no smoke (session pattern)** — `pnpm typecheck` 4/4, `pnpm build` green (route table adds `/applications/renewals`, 5.49 kB, static; `/applications/local/[id]` unchanged 9.76 kB — no touch), lint n/a. Register roundtrip, 409 paths (open detention #32 / not-active #26), live fees, disabled-button titles, modal submit — all unverified at runtime.
- Inactive-gray pill vs ui-rules "Inactive→destructive" listing — decide whether to amend ui-rules.md or revert to red.
- DataTable `header`/`showSearch` now part of the shared component — 7.2 REVIEW should confirm no older consumer regressed (all defaults preserved).
- **REVIEWs still owed (user's call): 7.2 `[UI]` + 7.1 `[LOGIC]` (the mandated 7.1 REVIEW was skipped again this session per user's direct build order), then 6.1, 5.1, 5.2, backlog 0.B.2/0.C.1/1.1/4.1/4.2.**
- Carried: flagged concurrency hole (6.1 issuance vs 7.1 renewal same-class, no partial-unique-index backstop); pg deprecation warning; Completed-after-refresh banner license id/dates gap (register DTO now exists — 6.2 gap closable); Roles column data source; citizen-options 1000-window; TestType descriptions provisional.
**Start next session with:** Per AGENTS.md § 3.1 the REVIEW queue precedes the next feature — decide with the user: **REVIEW pass on 7.1 `[LOGIC]` + 7.2 `[UI]`** (cross-refs: #26 in-tx deactivate + race guard, #28 both fee snapshots AND the modals' live-fee display, #29 session user, #32 real guard + visibly-disabled rows, #9 one register DTO, #11 toDto gates, comment policy, DataTable extension regression check), or go straight to **8.1 — International License Service `[LOGIC]`** per user directive.

### Session 18 — 2026-08-19
**Completed:** **7.1 — License Renewal & Replacement `[LOGIC]`** (full vertical slice: MCP migration, new detain-release module, shared register DTO, 2 request DTOs, applications-service create helper, repository register read, service renewOrReplace, new licenses controller, module wiring; no UI code touched).
**Decisions made:**
- **DetainedLicenses table + minimal `detain-release/` module created NOW (user-approved, ARCHITECT)** — invariant #32 is absolute and 9.1 hasn't shipped; the guard must be real. MCP migration `create_detained_licenses_table` (full DBML shape + LicenseID index); module holds the `DetainedLicense` entity + `hasOpenDetention(manager, licenseId)` manager-parameter method. Session 16 drivers-early precedent; 9.1 builds service/controller/DTOs on it.
- **License id rides the URL; body carries only what varies (deviation from plan's `RenewalReplacementRequestDto(existingLicenseId, reason)`)** — `POST /licenses/:id/renew` (body `{ notes? }`), `POST /licenses/:id/replace` (body `{ reason: 'damaged'|'lost', notes? }`); 5.1 "id rides the URL" precedent; per-endpoint DTOs make reason/route mismatch impossible. **User amendment: notes field on both DTOs → new license's `Notes`** (≤500, mirrors IssueLicenseRequestDto).
- **Renewal application created Status=Completed directly** — create + resolution are one transaction; New would never be observable and must never be cancellable (lastStatusDate stamped at creation). Generic `createInTransaction(manager, params)` added to LocalLicenseApplicationsService (completeInTransaction precedent — licenses module still never imports the Application entity).
- **Register paginated + new shared `LicenseRegisterRowDto`** (LicenseDto minus applicationId + computed `isDetained`) — deviation from plan-literal "every local license"; consistent with every other DataTable register, includes deactivated rows (history/10.1 reuse). Repository: joinedQb + addSelect EXISTS open-detention, getRawAndEntities zip, count+page share one qb.
- **Double-renewal race guard**: deactivate via `UPDATE License SET IsActive=false WHERE id=:id AND IsActive=true`; `affected !== 1` → 409 — atomic in-tx, catches concurrent renewals hitting 0 rows.
- **Gates order:** 404 license (findById with joins) → 409 not-active (cheap pre-tx) → 404 application-type (reason-mapped title, fee source) → **tx**: 409 open-detention (inv #32) → create Completed application (ApplicationFees snapshot) → deactivate old → insert new license (same driver+class, fresh dates = IssueDate + DefaultValidityLength, ClassFees snapshot, IssueReason 2/3/4, session user) → reload + toDto.
- **Two controllers in the licenses module** — existing `LicensesController` keeps the foreign-prefixed issue-license route; new `LicensesRegisterController` (`@Controller('licenses')`) owns register/renew/replace.
**Deviations from plan:** as above — detention domain pulled early (with explicit user approval); DTO split with URL-carried id + notes fields (user amendment); paginated register; application created Completed.
**Known issues / follow-ups:**
- **No API boot + no smoke (user directive)** — `pnpm typecheck` 4/4, `pnpm build` green (3 packages; web route table unchanged), lint clean. The 3 new routes, register EXISTS flag, all 409 paths, and the MCP migration unverified at runtime.
- **Flagged concurrency hole:** a concurrent 6.1 issuance and a 7.1 renewal of the same class could each commit an active same-class license (guards are in-tx reads/updates only; no partial-unique-index backstop keyed on active rows) — REVIEW candidate, not fixed this session.
- **REVIEWs still owed (user's call): 7.1 itself, then 6.1 (now has 6.2 + 7.1 consumers), 5.1, 5.2, backlog 0.B.2/0.C.1/1.1/4.1/4.2.**
- Carried: pg deprecation warning; TestType descriptions provisional; citizen-options 1000-window; Roles column data-source decision; Completed-after-refresh banner license id/dates gap.
**Start next session with:** **REVIEW pass on 7.1 `[LOGIC]`** (cross-refs: #26 in-tx deactivate + race guard, #28 both fee snapshots, #29 session user, #32 guard real, #9 single register DTO, #11 toDto gates, transaction atomicity, comment policy), then decide with the user: **7.2 — License Renewal & Replacement `[UI]`** (ARCHITECT first: `/applications/renewals` page + LicenseRegisterTable with three action buttons, Detained rows visibly disabled, confirmation modals with live lookup fees, hooks + keys factory), or clear the REVIEW queue first.
