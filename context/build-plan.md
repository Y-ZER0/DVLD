...
**Do not start a `[UI]` sub-task until every box in its paired `[LOGIC]`
sub-task is checked and REVIEW has signed off** (`AGENTS.md § 3.1`).
...
- [ ] "Roles" column derived client-side from related User/Driver presence
      (read-only display, not stored on `Person`)
...
### 2.1 — [LOGIC]
- [ ] `UsersRepository`: `findAll` (joined with `Person` for display), `findByUsername`
      (with password, for auth only), `create`, `updatePassword`, `setActive`, `remove`
- [ ] `CreateUserRequestDto` (personId, username, password — hashed in service)
- [ ] `UsersService`: reject linking a `Person` who already has a `User` row;
      `toDto()` never includes the password hash
- [ ] `UsersController`: `GET /users`, `POST /users`,
      `PATCH /users/:id/password`, `PATCH /users/:id/status`, `DELETE /users/:id`
- [ ] `GET /people/unlinked` (or a query param on the people endpoint) to
      power the "Link to Person" picker — only people without a `User` row
